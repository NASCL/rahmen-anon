...
import utils
from model.rahmen import RAHMeN, train_model, eval_model
...
def main(args):
    ...
    out_model_dir = f'saved_models/RAHMeN/{args.dataset}{suffix}/{args.model_name}'
    os.makedirs(out_model_dir, exist_ok=True)

    log_path = f'logs/{args.dataset}{suffix}/{args.model_name}'
    log_fname = f'{log_path}/rahmen_log.out'
    ...
    if args.inductive:
        ...
        test_G = None
        train_G, val_edges, test_edges = utils.load_dataset(args.dataset, holdout_split=args.cv_split)

    if args.motif_feat:
        motif_file = f'motif_feat_{args.cv_split}.pt' if args.cv_split else 'motif_feat.pt'
        motif_feat = torch.load(f'data/{args.dataset}/{motif_file}')
        # train_G.ndata['feat'] = motif_feat
        train_G.ndata['feat'] = torch.cat(
            [train_G.ndata['feat'].unsqueeze(1).repeat(1, len(train_G.etypes), 1),
             motif_feat],
            dim=2
        )
    end = time()
    ...
    val_aucs, val_f1s, val_prs = train_model(
        model=model,
        train_G=train_G,
        val_edges=val_edges,
    ...
    test_aucs, test_f1s, test_prs = eval_model(
        model,
...
def parse_args():
    ...
    parser.add_argument('--motif-feat', action='store_true', default=False,
                        help='Concat motif counts to node input features.')
